...
-- make low rez proxy, thumbnail, and sidecar from video; probe original and proxy for metadata.
-- create sample frames from video and send to AWS for analysis.
...
function plugin.execute()
    ...
    local thumbnail_path = rio_utils.create_thumbnail_name(input, settings.thumbnail_format or "webp", working_directory)
    local sidecar_path = rio_utils.create_sidecar_name(input, settings.sidecar_format or "webp", working_directory)
    ...
    if not proxy_meta then
        ...
        -- rest of pipline will fail if proxy is not created, so return early
        return
    else
    ...
    -- coalesce and save technical metadata
