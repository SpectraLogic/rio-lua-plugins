...
---@param input_path string  source media file -- generally the proxy
---@param wav_path string  destination .wav path (build with rio_utils.create_wav_filename)
...
---@return string? err
local function transcribe_audio(input_path, wav_path, opts)
    opts = opts or {}
    ...
    local out_base = (wav_path:gsub("%.wav$", ""))   -- whisper-cli appends the .txt extension
...
---@field transcribe_audio fun(input_path: string, wav_path: string, opts?: table): table|nil, string? # Transcribe media to plain text via whisper-cli.
